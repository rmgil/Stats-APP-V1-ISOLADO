...
def api_dashboard_token(token):
    ...
    Query params:
    - include_months: bool - Include monthly breakdowns (default: False)
    - month: str - Filter to specific month YYYY-MM (requires include_months=true)
    """
    try:
        include_months = request.args.get('include_months', 'false').lower() == 'true'
        month_filter = request.args.get('month', None)
        
        data = build_dashboard_payload(token, include_months=include_months, month_filter=month_filter)
        return jsonify({
            "ok": True,
            "data": data
        })
...
def api_dashboard_payload():
    ...
    out = build_dashboard_payload(token, include_months=include_months, month_filter=month_filter)
    return jsonify(out)
...
def download_hands_by_stat_with_format(token, format_name, stat_filename):
    ...
    try:
        ...
        if month_param and not re.match(r'^\d{4}-\d{2}$', month_param):
        ...
        # Build path based on month parameter
        if month_param:
            storage_path = f"/results/{token}/months/{month_param}/hands_by_stat/{format_name}/{stat_filename}"
        else:
        ...
        # Fallback to local filesystem (for backwards compatibility)
        if month_param:
            file_path = os.path.join("work", token, "months", month_param, "hands_by_stat", format_name, stat_filename)
        else:
...
def list_hands_by_stat(token):
    ...
    # Get optional month parameter (format: YYYY-MM)
    month_param = request.args.get('month', '').strip()
    
    app.logger.info(f"🔍 list_hands_by_stat called for token: {token}, month: {month_param or 'aggregate'}")
    try:
        # Validate token
        if not re.match(r'^[a-f0-9]{12}$', token):
        ...
        # Validate month format if provided
        if month_param and not re.match(r'^\d{4}-\d{2}$', month_param):
            app.logger.warning(f"Invalid month format: {month_param}")
            return jsonify({"error": "Invalid month format. Use YYYY-MM"}), 400
        ...
        # Build path based on month parameter
        if month_param:
            # Monthly path: /results/{token}/months/{month}/hands_by_stat
            stats_dir_cloud = f"/results/{token}/months/{month_param}/hands_by_stat"
            stats_dir_local = os.path.join("work", token, "months", month_param, "hands_by_stat")
        else:
            # Aggregate path: /results/{token}/hands_by_stat
                    ...
                    if os.path.exists(full_path):
                        for filename in os.listdir(full_path):
                            if filename.endswith('.txt'):
                                ...
                                # Build download URL with month parameter if provided
                                download_url = f"/api/download/hands_by_stat/{token}/{group}/{filename}"
                                if month_param:
                                    download_url += f"?month={month_param}"
