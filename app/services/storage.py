...
import shutil
from .object_storage import get_object_storage, ObjectNotFoundError
...
class StorageService:
    ...
    def __init__(self, local_base_dir: str = "/tmp/storage", require_cloud: bool = False):
        """
        Initialize storage service
        
        Args:
            local_base_dir: Base directory for local filesystem fallback
            require_cloud: If True, fail fast if Object Storage is not available (production mode)
        """
        ...
        # TEMPORARILY DISABLED: Object Storage has authentication issues
        # Force local storage until Object Storage is properly configured
        # TODO: Re-enable once Replit sidecar authentication is fixed
        self.use_cloud = False  # self.object_storage.is_available()
        
        # Fail fast if cloud storage required but not available
        if require_cloud and not self.use_cloud:
            raise RuntimeError(
                "Object Storage required but not available. "
                "Make sure you're running on Replit with Object Storage configured."
            )
        
        is_production = os.getenv('REPL_DEPLOYMENT') == '1'
        env_name = "PRODUCTION" if is_production else "DEVELOPMENT"
        if self.use_cloud:
            logger.info(f"Storage: Using Object Storage (cloud mode - {env_name})")
        else:
    ...
    def _cloud_path(self, path: str) -> str:
        """
        Convert storage path to cloud storage path
        
        Format: /bucket_name/path/to/file
        We use environment variable PRIVATE_OBJECT_DIR as bucket prefix
        """
        # Get bucket prefix from environment
        bucket_prefix = os.getenv('PRIVATE_OBJECT_DIR', '/poker-hands-storage/files')
        
        # Ensure path starts with slash
        if not path.startswith('/'):
            path = f'/{path}'
        
        # Combine bucket prefix with path
    ...
    def upload_file(self, file_data: bytes, path: str, content_type: str = 'application/octet-stream') -> None:
        ...
        try:
            if self.use_cloud:
                ...
                self.object_storage.upload_file(file_data, cloud_path, content_type)
    ...
    def upload_file_stream(self, file_stream: BinaryIO, path: str, content_type: str = 'application/octet-stream') -> None:
        ...
        try:
            if self.use_cloud:
                ...
                cloud_path = self._cloud_path(path)
                self.object_storage.upload_file_stream(file_stream, cloud_path, content_type)
                logger.debug(f"Uploaded stream to cloud: {path}")
            else:
    ...
    def download_file(self, path: str) -> Optional[bytes]:
        ...
        try:
            if self.use_cloud:
                ...
                cloud_path = self._cloud_path(path)
                data = self.object_storage.download_file(cloud_path)
                logger.debug(f"Downloaded from cloud: {path} ({len(data)} bytes)")
                return data
            else:
            ...
            logger.warning(f"File not found in cloud: {path}")
            return None
        except Exception as e:
    ...
    def download_file_stream(self, path: str) -> Optional[BinaryIO]:
        ...
        try:
            if self.use_cloud:
                ...
                cloud_path = self._cloud_path(path)
                stream = self.object_storage.download_file_stream(cloud_path)
                logger.debug(f"Downloaded stream from cloud: {path}")
                return stream
            else:
    ...
    def delete_file(self, path: str) -> bool:
        ...
        try:
            if self.use_cloud:
                ...
                cloud_path = self._cloud_path(path)
                result = self.object_storage.delete_file(cloud_path)
                logger.debug(f"Deleted from cloud: {path}")
                return result
            else:
    ...
    def file_exists(self, path: str) -> bool:
        ...
        try:
            if self.use_cloud:
                cloud_path = self._cloud_path(path)
                return self.object_storage.file_exists(cloud_path)
            else:
                local_path = self._local_path(path)
