...
def _track_discard(discard_stats: Dict, hand_text: str, reason: str):
    """
    Helper to track a discarded hand both globally and per-month.
    
    Args:
        discard_stats: The discard statistics dictionary to update
        hand_text: Raw hand history text (for month extraction)
        reason: Discard reason key (e.g., 'mystery', 'less_than_4_players')
    """
    # Update global counter
    discard_stats[reason] += 1
    
    # Extract month and update per-month counter
    month = extract_month_from_hand(hand_text) or 'undated'
    
    if month not in discard_stats['per_month']:
        discard_stats['per_month'][month] = {
            'mystery': 0,
    ...
    discard_stats['per_month'][month][reason] += 1
...
def classify_hands_individually(content: str, filename: str) -> Tuple[List[Dict], Dict]:
    ...
    discard_stats = {
        ...
        'per_month': {}  # Track discards per month (YYYY-MM)
    }
    ...
    for idx, hand_text in enumerate(hands):
        ...
        if is_cash_game(hand_text):
            _track_discard(discard_stats, hand_text, 'cash_game')
            continue
        
        # If filename contains 'mystery', discard all hands from this file
        if is_mystery_file:
            _track_discard(discard_stats, hand_text, 'mystery')
            continue
        ...
        if table_format == 'discard':
            _track_discard(discard_stats, hand_text, 'less_than_4_players')
            continue
        ...
        if tournament_type == 'PKO':
            ...
            if table_format == '6-max':
                ...
                _track_discard(discard_stats, hand_text, 'invalid_format')
                continue  # Skip if can't classify
        else:
            _track_discard(discard_stats, hand_text, 'other')
            continue  # Skip if can't classify
    ...
    total_discarded = sum(v for k, v in discard_stats.items() if k not in ['total_segments', 'per_month'] and isinstance(v, (int, float)))
...
def process_files_hand_by_hand(input_dir: str, output_dir: str, token: Optional[str] = None) -> Dict:
    ...
    for file_idx, txt_file in enumerate(all_files, 1):
        ...
        for discard_type, count in file_discard_stats.items():
            if discard_type == 'total_segments':
                continue  # Don't add total_segments to discarded_hands
            elif discard_type == 'per_month':
                # Merge per-month dictionaries separately
                for month, month_counts in count.items():
                    if month not in stats['discarded_hands']['per_month']:
                        stats['discarded_hands']['per_month'][month] = {
                            'mystery': 0,
                            'less_than_4_players': 0,
                            'tournament_summary': 0,
                            'cash_game': 0,
                    ...
                    for reason, reason_count in month_counts.items():
                        stats['discarded_hands']['per_month'][month][reason] += reason_count
            else:
                # Add numeric counters normally
    ...
    # Calculate the total discarded (excluding total_segments and per_month dict)
    stats['discarded_hands']['total'] = sum(
        v for k, v in stats['discarded_hands'].items() 
        if k not in ['total', 'total_segments', 'per_month'] and isinstance(v, (int, float))
    )
    ...
    # Sum all discarded hands (excluding 'total' which is already a sum, and per_month dict)
    total_discarded = sum(
        v for k, v in stats['discarded_hands'].items() 
        if k not in ['total', 'total_segments', 'per_month'] and isinstance(v, (int, float))
    )
